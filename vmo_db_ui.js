...
(() => {
  ...
  async function authenticatedJsonHeaders() { const user = window.VMOFirebase?.getCurrentUser?.(); if (!user) throw new Error('Vui lòng đăng nhập Google trước khi dùng AI.'); return { 'Content-Type': 'application/json', Authorization: `Bearer ${await user.getIdToken()}` }; }
  ...
  function showToast(message, isSuccess = true) {
    ...
    toast.textContent = `${isSuccess ? '✅' : '⚠️'} ${String(message ?? '')}`;
  ...
  window.removeSelectedImage = function() {
    ...
    showToast('Đã hủy ảnh bài giải đã chọn.', true);
  };
...
Vậy giới hạn cần tìm là $\\sqrt{2}$.`;
  ...
  // Định dạng nội dung Markdown an toàn (hỗ trợ cả mảng, đối tượng, chuỗi)
  function formatMathMarkdown(val) {
    if (val == null) return '';
    if (Array.isArray(val)) {
      return val.map(item => formatMathMarkdown(item)).join('<br>');
    }
    if (typeof val === 'object') {
      return Object.entries(val).map(([k, v]) => `<strong>${k}:</strong> ${formatMathMarkdown(v)}`).join('<br>');
    }
    const str = escapeHtml(val);
    return str
        ...
        <div style="display: flex; align-items: flex-start; gap: 12px; margin-bottom: 12px;">
          ...
          <div>
            ...
            <p style="margin: 0; color: #7f1d1d; font-size: 0.88rem; line-height: 1.5;">
              ${escapeHtml(errorMsg || 'Kết nối tới dịch vụ AI gặp trục trặc hoặc model quá tải.')}
            </p>
          </div>
        </div>
  ...
  window.switchSubmissionTab = function(tabName) {
    ...
    if (tabName === 'eval') {
      ...
      if (evalContent && window.MathJax?.typesetPromise) {
        window.MathJax.typesetPromise([evalContent]).catch(() => {});
      }
    } else {
  ...
  window.printEvaluationReport = function() {
    ...
    printWin.document.write(`
      ...
      <head>
        <meta charset="utf-8">
        <title>Báo cáo thẩm định bài giải VMO - ${subData.problemTitle || 'Bài toán'}</title>
        <style>
      ...
      <body>
        ...
        <p><strong>Bài toán:</strong> ${subData.problemTitle || 'Bài tập VMO'}</p>
        <div class="banner">
          <p><strong>Kết luận:</strong> ${evalData.verdictLabel || evalData.verdict || 'Hoàn tất'}</p>
          <p><strong>Điểm ước lượng:</strong> ${evalData.estimatedScore || '5.0/5.0đ'}</p>
        </div>
        <div class="card"><div class="title">1. Nhận định tổng quan:</div><div>${evalData.summary || ''}</div></div>
        <div class="card"><div class="title">2. Hướng tiếp cận & Phương pháp:</div><div>${evalData.approachAnalysis || ''}</div></div>
        <div class="card"><div class="title">3. Rà soát chi tiết từng bước:</div><div>${evalData.stepByStep || ''}</div></div>
        <div class="card"><div class="title">4. Lỗ hổng logic / Lưu ý:</div><div>${evalData.criticalFlaws || ''}</div></div>
        <div class="card"><div class="title">5. Lời khuyên của Chuyên gia:</div><div>${evalData.recommendations || ''}</div></div>
      </body>
      </html>
    `);
  ...
  function generateEvaluationReportHtml(evalData, isFullView = false) {
    const verdictBg = evalData.verdictColor || (evalData.verdict === 'CORRECT' ? '#16a34a' : (evalData.verdict === 'LOGICAL_GAP' ? '#ea580c' : (evalData.verdict === 'INCORRECT' ? '#dc2626' : '#6366f1')));
      ...
      <div style="padding: 16px 20px; background: ${verdictBg}; color: #ffffff; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 12px; border-radius: 10px 10px 0 0;">
        <div>
          ...
          <div style="font-size: 1.15rem; font-weight: 800; text-shadow: 0 1px 2px rgba(0,0,0,0.2);">
            ${evalData.verdictLabel || evalData.verdict || 'ĐÚNG HOÀN TOÀN (TỐI ƯU)'}
          </div>
        </div>
        <div style="background: rgba(255,255,255,0.25); border: 1.5px solid rgba(255,255,255,0.5); border-radius: 8px; padding: 6px 16px; font-weight: 800; font-size: 1.15rem; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
          Điểm: ${evalData.estimatedScore || '5.0/5.0đ'}
        </div>
      </div>
  ...
  function displayEvaluationResult(evalData) {
      ...
      noticeEl.innerHTML = `
        <div style="display: flex; align-items: center; gap: 8px;">
          ...
          <div>
            ...
            <span>${evalData.verdictLabel || 'Đã phân tích xong'} (${evalData.estimatedScore || ''})</span>
          </div>
        </div>
    ...
    // Kích hoạt MathJax hiển thị công thức toán
    if (window.MathJax?.typesetPromise) {
      const targets = [document.getElementById('tabEvaluationMainContent'), resultBox].filter(Boolean);
      if (targets.length > 0) {
    ...
    if (submitBtn) {
      submitBtn.onclick = async () => {
        ...
        try {
          ...
          showToast('Lỗi lưu bài giải: ' + (err.message || 'Không thể ghi vào database'), false);
        } finally {
  ...
  window.previewMathJaxSolution = function() {
    const text = document.getElementById('subSolutionText').value;
