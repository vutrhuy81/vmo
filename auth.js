...
const VMOAuth = (() => {
  ...
  // Local accounts are retired; Firebase Google is the only identity provider.
  const DEFAULT_USERS = [];
  ...
  function getSession() {
    try {
      ...
      if (!sess || !sess.username || sess.provider !== 'google' || !sess.uid) return null;
      return sess;
    } catch (e) {
      return null;
    }
  ...
  function login(username, password, remember = true) {
    return { success: false, message: 'Đăng nhập cục bộ đã bị vô hiệu hóa. Vui lòng dùng Google.' };
    /* legacy local login is intentionally unreachable
    initStorage();
